...
/*   Updated: 2023/02/06 01:18:05 by acardona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*static void	fts_init_post_frames_init(t_global *glo, char debug)
{
	...
	frame1->frame->img = mlx_new_image(glo->mlx, glo->map_w * CHUNK_SIZE,
			glo->map_h * CHUNK_SIZE);
	if (!(frame1->frame->img))
		ft_garbage_group_free(&(glo->garb.gbgroup), ERR_MLX);
	ft_garbage_add(glo->garb.gbimg, frame1->frame->img);
	frame2->frame->img = mlx_new_image(glo->mlx, glo->map_w * CHUNK_SIZE,
			glo->map_h * CHUNK_SIZE);
	if (!(frame2->frame->img))
		ft_garbage_group_free(&(glo->garb.gbgroup), ERR_MLX);
