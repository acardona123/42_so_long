...
/*   Updated: 2023/02/02 21:14:31 by acardona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_parsing_main(t_global *glo, char *map_name, char debug)
{
	...
	*fd = open(map_name, O_RDONLY);
	...
	ft_parsing_file_to_map(glo, fd, glo->map, debug);
	ft_parsing_check_map_content(glo, glo->map, debug);
	ft_parsing_check_map_path(glo, glo->map, debug);
	if (debug)
		write(1, " ok\n", 4);
}
