...
/*   Updated: 2023/02/11 22:32:17 by acardona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fts_game_open_exit(t_global *glo)
{
	...
	ft_tools_img_blend(glo->frames->frame, &glo->textures.exit_open->data,
		exit.x * CHUNK_SIZE, exit.y * CHUNK_SIZE);
	ft_tools_img_blend(glo->frames->next->frame, &glo->textures.exit_open->data,
		exit.x * CHUNK_SIZE, exit.y * CHUNK_SIZE);
	glo->exit_open = 1;
}
